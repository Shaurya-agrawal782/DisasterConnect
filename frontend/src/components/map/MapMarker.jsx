  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex items-center justify-center w-6 h-6">
        ...
        <span class="relative inline-flex rounded-full h-3.5 w-3.5 ${colorClass} border-2 border-slate-950 shadow-lg"></span>
      </div>
    `,
    iconSize: [24, 24],
    iconAnchor: [12, 12],
...
const getResourceIcon = (status) => {
  let colorClass = 'bg-emerald-500';
  let textColorClass = 'text-slate-950';
  if (status === 'assigned') {
  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex items-center justify-center w-6 h-6">
        <div class="w-5 h-5 rounded ${colorClass} ${textColorClass} flex items-center justify-center border-2 border-slate-950 shadow-xl">
...
export default function MapMarker({ type, data }) {
  ...
  const getStatusColor = (stat) => {
    switch (stat) {
      case 'reported': return 'bg-sky-500/10 text-sky-400 border-sky-500/20';
      case 'verified': return 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20';
      case 'assigned': return 'bg-violet-500/10 text-violet-400 border-violet-500/20';
  ...
  return (
    <Marker position={position} icon={icon}>
      <Popup className="custom-leaflet-popup">
        <div className="p-3 text-slate-200 bg-slate-950/95 border border-slate-800 rounded-lg max-w-[240px] space-y-2.5">
          {type === 'incident' ? (
            <>
              {/* Incident Popup Details */}
              <div className="space-y-1">
                <span className="text-[9px] uppercase tracking-wider text-rose-500 font-bold block">Incident Report</span>
                <h4 className="font-bold text-white text-sm leading-tight leading-normal m-0 p-0">{data.title}</h4>
              </div>

              <div className="grid grid-cols-2 gap-1.5 text-[10px] leading-normal pt-1">
                <div>
                  <span className="text-slate-500 block">Type:</span>
                  <span className="font-semibold text-slate-300 capitalize">{data.type}</span>
                </div>
                <div>
              ...
              <div className="text-[10px] text-slate-400 pt-1 border-t border-slate-900/60 leading-relaxed">
                <span className="font-semibold block text-slate-500 text-[9px] uppercase">Address</span>
            ...
            <>
              ...
              <div className="space-y-1">
                <span className="text-[9px] uppercase tracking-wider text-indigo-400 font-bold block">Emergency Resource</span>
                <h4 className="font-bold text-white text-sm leading-tight leading-normal m-0 p-0">{data.name}</h4>
              </div>

              <div className="grid grid-cols-2 gap-1.5 text-[10px] leading-normal pt-1">
                <div>
                  <span className="text-slate-500 block">Type:</span>
                  <span className="font-semibold text-slate-300 capitalize">{data.type.replace('_', ' ')}</span>
                </div>
                <div>
                  <span className="text-slate-500 block">Status:</span>
                  <span className={`px-1.5 py-0.5 rounded font-bold border capitalize inline-block ${getStatusColor(data.status)}`}>
                ...
                <div>
                  ...
                  <span className="font-semibold text-slate-300">{data.capacity}</span>
                </div>
              </div>

              <div className="text-[10px] text-slate-400 pt-1 border-t border-slate-900/60 leading-relaxed">
                <span className="font-semibold block text-slate-500 text-[9px] uppercase">Station Address</span>
